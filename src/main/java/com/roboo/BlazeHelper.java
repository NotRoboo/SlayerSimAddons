...
public class BlazeHelper {
    ...
    private static final double TARGET_Z         = 32.2;
    private static final double Z_TOLERANCE      = 0.1;

    private static final double TARGET_X         = -98.3;
    ...
    private static final float MID_YAW           = -50f;
    private static final float MID_PITCH         = 0f;
    private static final float FINAL_YAW         = 0f;
    private static final float FINAL_PITCH       = 0f;
    private static final float ROTATION_TOLERANCE = 0.01f;
    ...
    private static void handleMessage(String msg) {
        if (msg == null) return;
        if (!ModConfig.getPathfindingMode().equals("Blaze Warden")) return;
    ...
    private static void onTick() {
        if (mc.player == null || !ModConfig.getPathfindingMode().equals("Blaze Warden")) return;
        ...
        switch (stage) {
            ...
            case MOVE_FORWARD -> {
                if (MovementHelper.moveToZ(TARGET_Z, Z_TOLERANCE, 0.3)) {
                    stage = Stage.MID_ROTATE;
                }
            }

            case MID_ROTATE -> {
                boolean done    = RotationHelper.lookAt(MID_YAW, MID_PITCH);
                float yawDiff   = Math.abs(wrapDegrees(mc.player.getYRot() - MID_YAW));
                float pitchDiff = Math.abs(mc.player.getXRot() - MID_PITCH);
                if (done || (yawDiff < ROTATION_TOLERANCE && pitchDiff < ROTATION_TOLERANCE)) {
            ...
            case MOVE_FORWARD_2 -> {
                if (MovementHelper.walkForwardToX(TARGET_X, X_TOLERANCE)) {
